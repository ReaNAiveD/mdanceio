...
fn vs_main(
    ...
    vout.normal = vec3<f32>(1.0, 1.0, 1.0);
    vout.texcoord0 = vin.texcoord0.xy;
    vout.texcoord1 = vec2<f32>(0.5, 0.5);
    return vout;
}
...
fn fs_main(
    ...
    let rgb = vec3<f32>((fin.shadow0.z / fin.shadow0.w) * 0.5 + 0.5);
    return coverage_alpha(fin, vec4<f32>(rgb, 1.0));
}
